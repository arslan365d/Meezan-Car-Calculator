...
let car__delievery1 = document.querySelector("#car__delievery1")
...
async function calculateFunc(cardId) {
  ...
  if (Object.keys(selectedValuesObj).length === 0) {
    ...
    templateOfUIAfterClick.querySelector("#redoBtn").addEventListener("click", () => {
      // Here i first clone the html above then append it in card and call the function applyingEvents() to apply events on all dropdowns and after it selects all three card calculate btns and call the function calculateFunc to acutal calculate the given values and append it in card to show it to user.
      card.innerHTML = ""
...
function applyingEvents() {
  ...
  console.log(carField1,carField2,carField3);
  ...
  carVendor2.addEventListener("change", (e) => {
  ...
  carVendor3.addEventListener("change", (e) => {
...
applyingEvents(); // Call your function once the DOM is loaded
// Applying event on all three calculate Buttons
applyingCalculateEvent()
...
// At Completion of code, lines are 380 and now it is 261.